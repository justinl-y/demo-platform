...
async function refresh(repository: AuthenticationRepository, jwt: JWT, params: RefreshParams): Promise<RefreshResult> {
  const {
    accessTokenJwt,
    refreshTokenJwt,
  } = Config.authConfig();
...
async function logout(repository: AuthenticationRepository, jwt: JWT, params: LogoutParams): Promise<LogoutResult> {
  const {
    refreshTokenJwt,
  } = Config.authConfig();
...
async function sendPasswordResetEmail(repository: AuthenticationRepository, params: SendPasswordResetEmailParams): Promise<void> {
  const {
    email,
    actionUrl,
    userId,
...
async function passwordForgot(repository: AuthenticationRepository, params: PasswordForgotParams): Promise<void> {
  const {
    email,
  } = params;
...
async function passwordReset(repository: AuthenticationRepository, params: PasswordResetParams): Promise<void> {
  const {
    passwordResetToken,
    newPassword,
  } = params;
